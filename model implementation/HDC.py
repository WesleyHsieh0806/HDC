...
class HDC:
    ...
    def test(self, test_x):
        ...
        # 用 map function他就會自動分配資料讓核心去跑
        start = time.time()
        self.encoder_query_vector(test_x, query_vector, y_pred)
        end = time.time()
        print(end-start)
        self.y_pred = y_pred
        return y_pred
    ...
    def encoder_query_vector(self, test_x, query_vector, y_pred):
        ...
        for data in range(len(test_x)):
            ...
            for feature in range(1, self.nof_feature+1):
                ...
                if test_x[data, feature-1] > self.maximum[feature-1]:
                    test_x[data, feature-1] = self.maximum[feature-1]
                elif test_x[data, feature-1] < self.minimum[feature-1]:
                    test_x[data, feature-1] = self.minimum[feature-1]

                # 先看這個數值跟這個feature的minimum差多少  算出他的lev 藉此給他相對應的CIM vector
                lev = (test_x[data, feature-1] - self.minimum[feature-1]
                       )//((self.difference[feature-1])/self.level)
                query_vector[data] += self.IM_vector['feature' +
                                                     str(feature)] * self.CIM_vector[0 + lev]

                if feature == 1 and (self.nof_feature % 2) == 0:
                    # 因為maximum minimum使用的是train data的資料 要小心超過範圍
                    if test_x[data, feature] > self.maximum[feature]:
                        test_x[data, feature] = self.maximum[feature]
                    elif test_x[data, feature] < self.minimum[feature]:
                        test_x[data, feature] = self.minimum[feature]

                    # 當self.nof_feature有偶數個 需要補上E1*V1*E2*V2項
                    LEV = (test_x[data, feature] - self.minimum[feature]
            ...
            y_pred[data] = self.most_similar_class(query_vector[data])
...
if __name__ == "__main__":
    ...
    y = np.arange(10).astype(int).reshape(10, 1)
    test_x = np.random.rand(2, 4)
    a = HDC(6, 10, 4, 21)
    a.train(x, y)
    y_pred = a.test(test_x)
    a.result_to_csv('./prediction.csv')
    print(y_pred)
